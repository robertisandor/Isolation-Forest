...
class IsolationTreeEnsemble:
    ...
    def path_length(self, X:np.ndarray) -> np.ndarray:
        ...
        """
        # values = [[self.instance_path_length(X[idx, :], 
        #                                             tree.root, 
        #                                             height_limit=tree.height_limit, 
        #                                             e=0, 
        #                                             total_size=len(X)) 
        #                     for tree in self.trees] for idx in range(len(X))]
        # avg_values = [( np.mean([item[0] for item in row]), np.mean([item[1] for item in row]) ) for row in values]
    ...
    def anomaly_score(self, X:np.ndarray) -> np.ndarray:
        ...
        scores = [2**(-1 * avg_path_lengths[idx] / c_n) for idx, x_i in enumerate(X)]
        #print(scores)
        #print(len([score for score in scores if score < 0.5]), len([score for score in scores if score >= 0.5]))
        #return 
    ...
    def fit(self, X:np.ndarray, improved=False):
        ...
        if improved:
            ...
            if self.height_limit <= 0 or X.shape[0] <= 1:
                # TODO: needs to be revised probably needs to return immediately
...
def find_TPR_threshold(y, scores, desired_TPR):
    ...
    while TPR < desired_TPR:
        thresholded_scores = np.array([1 if score <= score_threshold else 0 for score in scores])
        TP = np.sum([1 for y, y_hat in list(zip(y, thresholded_scores)) if y == 1 and y_hat == 1])
        FP = np.sum([1 for y, y_hat in list(zip(y, thresholded_scores)) if y == 0 and y_hat == 1])
        TPR = TP / float(len(scores))
        print(f"TPR = {TPR}")
        FPR = FP / float(len(scores))
        print(f"FPR = {FPR}")
